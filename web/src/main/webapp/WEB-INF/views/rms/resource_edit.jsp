...
$(document).ready(function(){
	$("#resourceUpdateForm").form('load','${ctx}/rms/resource/preedit?id='+$("#id").val());
	$('#btnSubmit').click(function(){
		if($('#resourceUpdateForm').form('validate')){
			$.ajax({
				type:'post',
				dataType:'json',
				url:'${ctx}/rms/resource/update',
				data:$('#resourceUpdateForm').serialize(),
				success:function(data){
					if(!data.success){
						$.messager.alert('提示',data.message,'info');
					}else {
						window.parent.$('#winEdit').window('close');
...
<body>
<form id="resourceUpdateForm" method="post">
...
<table class="form_table" align="center">
	...
	<tr>
		<td><label for="parentId">父节点:</label></td>
       	<td><select id="pidSelect" class="easyui-combotree" name="parentId" data-options="url:'${ctx}/rms/resource/getAllResource',panelHeight:'auto',width:'170px'"></td>
        <td></select>
	</tr>
	<tr>
